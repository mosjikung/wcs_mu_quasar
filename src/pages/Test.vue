...
<q-select
  ...
  <div class="col-2">
  <q-btn color="light-green-9" label="export" @click="exportexcel()"></q-btn>
  </div> 
  <div class="col-2">
  <q-btn color="light-green-9" label="check_data" @click="checkdata()"></q-btn>
  </div> 
  </div>
</template>
  ...
  data(){
    ...
    methods:{
      ...
      let org = this.$q.localStorage.getItem("org");
       worksheet.getCell("A1").value = "LOSS AUDIT DAILY REPORT  ( MU-SLA-1 ) : NY"+org //ส่วนหัว
      ...
      worksheet.getCell("A3").value = "Date"; //ส่วนหัว
       ...
       worksheet.getCell("B3").value = "00-Jan-00";
      ...
      worksheet.getCell("F3").value = "Fabrice Type";
      ...
      worksheet.getCell("A1").value = "LOSS AUDIT DAILY REPORT  ( MU-SLA-1 ) : NY"+org; //ส่วนหัว
       ...
       for( x=0; x<this.multiple.length;   x++){
        ...
        if(this.multiple[x] == "Light-Middle Weight"){
          this.chosen = 2
        }
        if(this.multiple[x] == "Fleece"){
          this.chosen = 3
        }
        if(this.multiple[x] == "Special"){
          this.chosen = 4
        }
...
//L------------------------------------------------------------------------------------
 ...
 worksheet.getCell("S"+m).value = 0
  ...
  worksheet.getCell("T"+m).value = 0
    ...
    worksheet.getCell("A78").value = "50";
      worksheet.getCell("A78").alignment = { horizontal: "center",vertical:"middle" };
      worksheet.getCell("A78").border = {
        top: { style: "thin" },
        left: { style: "thin" },
        bottom: { style: "thin" },
        right: { style: "thin" },
      };
      worksheet.getCell("A79").value = Number(50);
      worksheet.getCell("A79").alignment = { horizontal: "center",vertical:"middle" };
      worksheet.getCell("A79").border = {
        top: { style: "thin" },
        left: { style: "thin" },
        bottom: { style: "thin" },
        right: { style: "thin" },
      };
      worksheet.getCell("A78").value = Number(60);
      worksheet.getCell("A78").alignment = { horizontal: "center",vertical:"middle" };
      worksheet.getCell("A78").border = {
        top: { style: "thin" },
        left: { style: "thin" },
      ...
      worksheet.getCell("A77").value = Number(40);
      worksheet.getCell("A77").alignment = { horizontal: "center",vertical:"middle" };
      worksheet.getCell("A77").border = {
        top: { style: "thin" },
        left: { style: "thin" },
        bottom: { style: "thin" },
        right: { style: "thin" },
...
/* 
      worksheet.getCell('A80').formula === 'A78+A79';
      worksheet.getCell('A81').result === '110'; */

      worksheet.getCell('A80').value = { formula: 'SUM(A77:A79)'};
